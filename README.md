...
1) Перебор элементов: Циклом for проходим по списку от начала до конца, используя индекс i.
2) Сравнение: Каждый элемент списка arr[i] сравнивается с искомым значением target.
3) Возврат результата: Если найден элемент, соответствующий цели, немедленно возвращаем его индекс.
4) Завершение поиска: Если пройден весь список и элемент не найден, возвращаем специальное значение (-1), обозначающее отсутствие такого элемента.
#### Временная сложность:
Последовательный поиск имеет следующие характеристики сложности:

**Наилучший случай:** O(1) — элемент найден первым (самый удачный сценарий).

**Наихудший случай:** O(n) — приходится пройти весь список целиком, потому что элемент либо расположен последним, либо вообще отсутствует.

**Средний случай:** также O(n) — ожидается, что придется просмотреть примерно половину элементов, хотя точные вероятности зависят от структуры данных.
## 9.Бинарный поиск.
